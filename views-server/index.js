// Self-hosted "profile views" counter.
// Same SVG design as the old Cloudflare Worker — just runs anywhere with Node
// and stores counts in a local JSON file instead of Workers KV.
//
// Run:   node index.js
// Env:   PORT (default 3000)
//        ALLOWED_IDS   comma-separated list of ids allowed to be counted
//                       (leave empty to allow any id)
//        DATA_FILE     path to the JSON file used for storage (default ./data/views.json)
...
const DATA_FILE = process.env.DATA_FILE || path.join(__dirname, "data", "views.json");
const ALLOWED_IDS = (process.env.ALLOWED_IDS || "")
  .split(",")
  .map((entry) => clean(entry.trim()))
  .filter(Boolean);
...
function renderCounter(count, id) {
...
function clean(value) {
  return value.toLowerCase().replace(/[^a-z0-9-_]/g, "").slice(0, 39);
}

// --- storage: a plain JSON file on disk instead of Cloudflare Workers KV ---
...
function saveStore(store) {
  ...
  fs.renameSync(tmp, DATA_FILE); // atomic-ish swap, avoids partial writes
}

// Simple in-process lock so two concurrent requests can't race on the same file.
...
// --- HTTP server (no framework needed, this is small enough for plain http) ---

const server = http.createServer(async (req, res) => {
  const url = new URL(req.url, `http://${req.headers.host}`);
  const [route, rawId] = url.pathname.split("/").filter(Boolean);
  const id = clean(rawId ?? "");

  if (route !== "views" || !id || (ALLOWED_IDS.length && !ALLOWED_IDS.includes(id))) {
  ...
  try {
    const count = req.method === "GET" ? await incrementCount(id) : currentCount(id);
    res.writeHead(200, HEADERS);
    res.end(req.method === "HEAD" ? undefined : renderCounter(count, id));
  } catch (error) {
    console.error(error);
...
server.listen(PORT, () => {
  console.log(`views counter listening on :${PORT}`);
  console.log(`try: http://localhost:${PORT}/views/your-github-username`);
});
